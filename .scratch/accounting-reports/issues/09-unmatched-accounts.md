...
- A test sums the top-level Categories and `unmatched` for every fixture, for a BalanceSheet (Result and scope included) and for a scoped ProfitAndLoss, and compares that to the ledger's own net read straight from the payload by an independent parser: nothing in scope can disappear.
- **Scope, added while doing it:** a ProfitAndLoss has no business with balance-sheet Accounts, so on the first local run against the real ledger every ProfitAndLoss listed its balance-sheet Accounts as "unmatched", which was noise. A template may declare a `scope` (CategoryRoots it answers for); only in-scope, unclaimed Accounts are unmatched. Both shipped templates declare one.
- **What the group found on the real ledger:** the French ProfitAndLoss template had no Category for classes 67/77 (exceptional), 68/78 (depreciation and provisions) or 69 (income tax). Added: _Depreciation and provisions_ under Operating expenses, _Exceptional_ and _Income tax_. Afterwards, checked locally, the group is empty for every FiscalYear and the ProfitAndLoss's Categories net to exactly the BalanceSheet's Result — the reconciliation the spec asks for.
